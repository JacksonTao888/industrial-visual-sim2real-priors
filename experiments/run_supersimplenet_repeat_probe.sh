...
OUT_ROOT="experiments/runs/no_cad/supersimplenet_repeat_probe"
...
conda run --no-capture-output -n sim2real python experiments/run_supersimplenet_anomalib.py \
  ...
  --max-epochs 999 \
  --max-steps 1000 \
  2>&1 | tee "${LOG_DIR}/mvtec_toothbrush_1000steps_repeat.log"
...
conda run --no-capture-output -n sim2real python experiments/run_supersimplenet_anomalib.py \
  ...
  --max-epochs 999 \
  --max-steps 3000 \
  2>&1 | tee "${LOG_DIR}/visa_macaroni2_capsules_3000steps_repeat.log"
