#!/usr/bin/env python3
"""Summarize per-category PatchCore metrics written by run_patchcore_anomalib.py."""
...
def summarize(dataset: str, result_dir: Path, rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "method_family": "Normal-only memory / PatchCore",
        "dataset": dataset,
...
def main() -> None:
    ...
    result_dir = args.result_dir or (
        args.project_root / "experiments" / "runs" / "no_cad" / "patchcore" / args.dataset
    )
    csv_path = args.csv or result_dir / "patchcore_results.csv"
    summary_path = args.summary or result_dir / "patchcore_results.summary.json"
