...
PRETRAINED="$RUN_DIR/tless_pbr_full_to_real_yolov8n_domain_randomization/weights/best.pt"

if [[ ! -f "$PRETRAINED" ]]; then
  echo "Missing pretrained domain-randomization checkpoint: $PRETRAINED" >&2
  exit 1
fi
...
yolo detect train \
  ...
  patience=15 \
  project="$RUN_DIR" \
  name=tless_dr_pretrained_real_5pct_finetune
